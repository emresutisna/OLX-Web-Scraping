  ...
  while True:
    ...
    for card in cards:
      if(card.attrs['data-make'].upper() in brands):
        try:
          itemPage = session.get(card.attrs['data-url'])
        except:
          print('Error')
          continue  
    ...
    if current_page == 1 :
      ...
      if idx == 0 :
        df.to_csv('mobil123_used_cars.csv', encoding="utf-8", index=False)
      else:
        df.to_csv('mobil123_used_cars.csv', encoding="utf-8", mode='a', header=False, index=False)
    else:
      df.to_csv('mobil123_used_cars.csv', encoding="utf-8", mode='a', header=False, index=False)
